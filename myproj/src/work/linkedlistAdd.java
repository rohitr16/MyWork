...
public class linkedlistAdd {
	...
	public static void addRemain(Node<Integer> head,Node<Integer> headTmp){
		
		if(head!=headTmp){
			addRemain(head.next,headTmp);
	...
	public static Node<Integer> linkedListAdd(Node<Integer> head1,Node<Integer> head2){
		...
		if(size1==size2){
			...
			if(size1>size2){
				...
				Node<Integer> curr=headTmp;
				headTmp=addList(headTmp, head2);
				addRemain(head1,curr);
				...
				addRemain(head2,curr);
				Node<Integer> tmp=head2;
				while(tmp.next!=curr){
					tmp=tmp.next;
				}
				tmp.next=headTmp;
